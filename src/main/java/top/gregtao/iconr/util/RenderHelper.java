...
public class RenderHelper {
    ...
    private Matrix4fStack modelStack;
    ...
    private void renderEntity(LivingEntity entity) {
        ...
        try {
            int size = (int) (11 / Math.max(entity.getWidth(), entity.getHeight()));
            InventoryScreen.drawEntity(context, 0, 0, size, size, size, 0.0F, 25.0F, 0.0F, entity);
        } catch (Exception e) {
    ...
    protected void renderGuiItemModel(ItemStack stack, int x, int y, BakedModel model, ItemRenderer renderer) {
        ...
        renderer.renderItem(stack, ModelTransformationMode.GUI, false, matrixStack2, immediate, 15728880, OverlayTexture.DEFAULT_UV, model);
        immediate.draw();
