...
public class IconRUtils {
    ...
    public static List<RecipeEntry<?>> getRecipesFromMod(ClientPlayerEntity player, String modId) {
        if (player.getWorld() == null) return List.of();
        List<RecipeEntry<?>> list = new ArrayList<>();
        player.getWorld().getRecipeManager().values().forEach(recipe -> {
    ...
    public static void resetLanguage(String lang) {
        ...
        if (!client.options.language.equals(lang)) {
            client.getLanguageManager().setLanguage(lang);
            client.options.language = lang;
